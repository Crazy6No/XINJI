...
public class PaySortAdapter extends RecyclerView.Adapter<PaySortAdapter.SortViewHolder> {

    private List<PaySortBean> mData = new ArrayList<>();

    public void setNewData(List<PaySortBean> data) {
        mData.clear();
        if (data.size() > 10) {
            List<PaySortBean> copy = data.subList(0, 10);
            mData.addAll(copy);
        } else {
            mData.addAll(data);
        }
    ...
    public void onBindViewHolder(@NonNull SortViewHolder holder, int position) {
        PaySortBean bean = mData.get(position);
        holder.tv_sort.setText(String.valueOf(position + 1));
        holder.tv_time.setText(bean.getPost());
        holder.tv_remake.setText(bean.getName());
        holder.tv_money.setText("¥" + bean.getData().getMoney());
        int color;
        if (position == 0) {
